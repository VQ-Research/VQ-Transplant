...
class AdversarialQuantizer(BaseQuantizer):
    ...
    def forward(self, z_enc):
        ...
        codebook_g_loss = self.calc_codebook_g_loss()
        loss = F.mse_loss(z_q, z.detach()) + self.args.gamma_2 * codebook_g_loss
...
class MultiscaleAdversarialQuantizer(MultiscaleBaseQuantizer):
    def __init__(self, args):
        super().__init__(args)
        self.args = args
    ...
    def forward(self, z_enc):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            loss =  multi_vq_loss + self.args.gamma_2 * codebook_g_loss
        return z_dec, loss, codebook_g_loss, quant_error, codebook_utilization, codebook_perplexity
