...
CUDA_VISIBLE_DEVICES="0,1" python -m torch.distributed.launch --nproc_per_node=2 --master_port=12273 train_PQ_transplant.py --VQ=ema_vq --dataset_name=ImageNet --path=vector --global_batch_size=64 --codebook_size 256  --codebook_dim=8 --pq=2 --stage=transplant --alpha=1.0 --beta=1.0 --gamma=0.0
